...
void GLWidget::changePositionLight() {
    ...
    cout << mon->getLlumActual()->getType()<<endl;
    updateGL();
...
void GLWidget::activaEnvMapping() {
    ...
    quinShader = 1;
    mon->conTextura = true;
    InitShader("://resources/vshaderPhong.glsl", "://resources/fshaderPhongEnviromental.glsl");
    updateGL();
}
...
void GLWidget::initializeGL() {
    ...
    Llum *dir = new Llum(Direccional);
    dir->setDirection(vec4(0.0,0.0,15.0));
    mon->addLlum(dir);
    Llum *spot = new Llum(SpotLight);
    spot->setAngle(1.14);
    mon->addLlum(spot);
...
void GLWidget::paintGL() {
    //qglClearColor(QColor(224,224,224,128));
    glClear( GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT );
    for(unsigned int i=0; i < mon->elements.size(); i++){
        mon->elements.at(i)->toGPU(program);
    }
    mon->draw();

    mon->llumsToGPU(program);
    mon->setAmbientToGPU(program);
    mon->setconTexturaToGPU(program);

    //mon->setconTexturaToGPU(program);
    //cout << mon->getLlumActual()->getAmbientIntensity() <<endl;
}
